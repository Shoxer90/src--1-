    ...
    <div style={{margin:"5px",letterSpacing:"1px"}}>
        ...
        {item?.saleType === 5  ? 
          <div> 
           <div> 
            ...
            <span>
              ...
              <span style={{margin:"0 5px"}}> {item?.cashAmount+ item?.cardAmount+item?.prePaymentAmount} {t("units.amd")}/</span> 
              {/* 12.09 */}
              {/* <span style={{margin:"0 5px"}}> {originTotal} {t("units.amd")}/</span>  */}
            </span>
            {item?.cashAmount ?
          ...
          <div>
            <strong>     
              <span>{t("history.receiptPrice2")}</span>
              <span> {item?.total} {t("units.amd")}waesrfghjgfrt</span>
            </strong>
            {item?.cashAmount ?
              <div > 
                {t("history.whichCash")}
