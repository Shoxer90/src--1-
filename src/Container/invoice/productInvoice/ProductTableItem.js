     ...
     {tableContent.map((record, index) => (
        <tr key={record.key} style={{ 
          ...
          <td style={cellStyle}>
            <input
              type="number"
              value={record?.dep===1 ? (record.price - record?.price * 0.1667).toFixed(2): record.price }
              style={{...inputStyle, width: '80px'}}
              readOnly
            /> 
          </td>
          ...
          <td style={cellStyle}>
           ...
           <td style={cellStyle}>
            <select
              ...
              disabled={true}
            >
              {taxType && taxType.map((type) => <option value={type?.id}>{type?.title}</option>)}
            </select>
          </td>
          ...
          <td style={cellStyle}>
            <input
              type="number"
              value={record?.dep===1 ? (record.count * (record.discountedPrice * 0.1667)).toFixed(2): 0}
              style={{...inputStyle, width: '70px'}}
              readOnly
            />
          </td>
