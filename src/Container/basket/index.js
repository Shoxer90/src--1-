...
import { basketListUrl, leftPrepaymentForProducts, payRequestQR, saleProductFromBasket } from "../../services/pay/pay";
...
// here I must check is it payment or prepayment sale
  const sale = async(saletype) => {
    let saleResponse = "";
    if(saletype === 1) {
      if(openWindow?.payment) {
        saleResponse = await saleProductFromBasket(paymentInfo)
      }else if(openWindow?.prepayment) {
        saleResponse = await leftPrepaymentForProducts(paymentInfo)
      }
    }else if(saletype === 2) {
      ...
      { loader ? <Loader />: 
        ...
        >
          <Box className={styles.bask_container}
            ...
            <PaymentContainer openWindow={openWindow} setOpenWindow={setOpenWindow} />
