...
import { untrackedProds } from '../untrackedProds';
...
const BascketContentItem = ({
  ...
  const [ isPrepaymentFinish, setIsPrepaymentFinish] = useState(!!localStorage.getItem("endPrePayment"))
  const ref = useRef();
  ...
  useEffect(() => {  
    setIsPrepaymentFinish(!!localStorage.getItem("endPrePayment"))
  },[]);
      ...
      {screen > 500 && <div className={styles.basketContent_item_image}>
        ...
        <div className={styles.basketContent_item_info}>
          ...
          <div style={{fontSize:"80%"}}>
            ...
            <div style={{fontSize:"90%"}}><strong>{el?.barCode? `bar ${el?.barCode} /  `: ""}</strong> <span> {el?.remainder ? `${t("productcard.remainder")} ${el?.remainder} ${t(`units.${el?.measure}`)}`: ""} </span> </div>
          </div> 
        </div>
        <div className={styles.basketContent_item_quantity}>
        {localStorage.getItem("endPrePayment") ? `${el?.count}`:
          <input
            ref={ref}
            ...
            onChange={(event) => {
              if(localStorage.getItem("endPrePayment")) {
                handleChangeForClosePrePayment(event)
              }else{
          ...
          <div style={{margin:"3px",width:"40px", fontSize:"80%"}}>
            {el?.measure ? t(`units.${el?.measure}`) : el?.unit ? t(`units.${el?.unit}`): ""}
          </div>
        </div>
        <div 
        ...
        >
          {localStorage.getItem("endPrePayment") ? "" : <DeleteIcon fontSize="medium" sx={{"&:hover":{color:"green"}}} />}
        </div>
        <ConfirmDialog 
